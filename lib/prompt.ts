...
function knowledgeBlock(): string {
  const types = KNOWLEDGE.map(
    (k, i) => `${i + 1}. ${k.title}：${k.desc} 典型信号：${k.signal}`,
  ).join("\n");
  const cases = CASES.map((c, i) => `${i + 1}. [${c.type}] ${c.method}`).join("\n");
...
const SYSTEM = `你是"岗位避坑检测器"的风险解释助手，服务于低经验的年轻求职者。
...
/**
 * 净化喂给 LLM 的参考文案：规则库内置解释里含"诈骗/骗局/违法"等定性词，
 * 直接喂会诱导 LLM 照搬越线。在此替换为中性表述，从源头堵住。
 */
